...
module.exports = function(html, options) {
  ...
  */
  ...
  html = html.replace(/(<[^\S\f]*\?[^\S\f]*xml\b[^>]*?\?[^\S\f]*>)/ig,
    function(str, p1) { return unprotectedTextAdd(p1); });
  // <% ... %>, <? ... ?>, <?php ... ?>
  html = html.replace(/(<[^\S\f]*(\%|\?)[\s\S]*?\2[^\S\f]*>)/g, // Not [^>]. '>' may be included.
    function(str, p1) { return protectedTextAdd(p1); });
  // <?php ...
  html = html.replace(/(<[^\S\f]*\?[^\S\f]*php\b[\s\S]*)/ig,
    function(str, p1) { return protectedTextAdd(p1); });
  // <jsp: ... >
  html = html.replace(/(<[^\S\f]*jsp[^\S\f]*:[^>]*?>)/ig,
    function(str, p1) { return protectedTextAdd(p1); });
  // <!--# ... -->
